...
#'
#' @param PPE.at.TS
#' A 4-column matrix generated with `PPESaggreg()`. No other objects are accepted.
...
#'
#' @details
#' The `SPEIChanges()` function implements a nonstationary parametric approach to detect
...
SPEIChanges <- function(PPE.at.TS, nonstat.models = 1){
  ...
  for (a in 1:48) {
    ...
    models <- Fit.Models(PPE.week, time,nonstat.models)
    ...
    if (Changes.Freq.Drought[a, 3]==1){
      ...
      sh <- models$parms[6]
      quasiprob.n <- matrix(NA,n.week,1)
      for (i in 1:n.week){
        quasiprob.n[i] <- pevd(PPE.week[i], loc = loc[i],
                               scale = sc[i],
                               shape = sh,
                               type = c("GEV"), lower.tail = TRUE, log.p = FALSE)}
      Changes.Freq.Drought[a, 4] <- round(stat.PPE.exp[1],2)
